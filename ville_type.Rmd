---
title: Analyse des données d'accidents 2011-2016 de la SAAQ.
...
if (ville != "province"){
  accidents <- accidents %>% filter(filter(NAME_MUNCP == ville))
}
...
save(velo, geo, file= paste0("geo_",ville,"_",type,".rdata"))
...
velo <- velo %>% filter(!is.na(lat) | !is.na(lon))
...
Il s'est produit `r accidents   %>% tally() `accidents impliquant un `r type` dans `r villetexte` entre 2011 et 2016. 
...
accidents %>% ggplot(aes(x=year))+ geom_histogram(binwidth = 1)+ 
  ggtitle(wrapper(paste0("Accidents impliquant des ", type, " dans la ", villetexte, " entre 2011 et 2016"),
                  width = 70))+ 
  theme_bw()
```
...
datatable(data =  accidents %>% 
          ...
          caption = paste0("Nombre d'accidents impliquant des ", type," dans chaque région administrative entre 2011 à 2016"))
```
...
datatable(data =  accidents %>% 
          ...
          rownames = F,
          options = list(dom = 't', ordering = F, pageLength=10) ,          
          caption = paste0("Nombre d'accidents impliquant des ", type," dans les 10 villes ayant eu le plus d'accidents entre 2011 à 2016"))
```
...
ggplot(accidents,aes(x = year, fill= gravite)) + 
  ...
  ggtitle(wrapper(paste0("Répartition des accidents (géolocalisés ou non) impliquant un ", type, " dans la ", villetexte, " selon la gravité de l'accident entre 2011 et 2016"), width=70))
```
...
datatable(data =  accidents %>% filter(gravite== "Mortel") %>% select(DT_ACCDN, gravite, location, NAME_MUNCP, NO_CIVIQ_ACCDN, RUE_ACCDN, ACCDN_PRES_DE, TP_REPRR_ACCDN, ACCDN_PRES_DEmod, ACCDN_PRES_DE)  %>%
          ...
          caption = paste0("Liste des ", accidents   %>% filter(gravite == "Mortel") %>% tally(), " accidents mortels  impliquant des ", type," dans la ", villetexte, ", 2011 à 2016"))
```
...
Le tableau  ci-bas montre le top 10 des intersections où se sont produits le plus d'accidents impliquant des `r type` dans la  `r villetexte ` entre 2011 et 2016.
...
worst10 <- velo %>% group_by(lat,lon) %>% slice(1:1) %>% ungroup() %>% select(accidents, location, lon, lat)  %>% arrange(desc(accidents))   %>% slice(1:10)


datatable(data = worst10  %>% select(-lat, -lon),
          rownames = F,
          options = list(dom = 't', ordering = F, pageLength=10) ,
          caption = paste0("Liste des 10 intersections comptant le plus d'accidents géolocalisés de ", type, " dans la ", villetexte," entre 2011 et 2016)"))
...
print( htmltools::tagList(datatable(data = worst10sub  %>% select(-lat, -lon),
          rownames = F,
          options = list(dom = 't', ordering = F, pageLength=10) ,          
          caption = paste0("Liste des 10 intersections comptant le plus d'accidents géolocalisés de ", type, " pour la ville de ", current$NAME_MUNCP," entre 2011 et 2016)"))
))
}
```
...
ACCDN_PRES_DE)) %>% select(-lat, -lon) ,
          ...
          caption = paste0("Liste des accidents survenus dans les 10 pires intersections de la ", villetexte, " entre 2011 et 2016"))
```
...
map_data <-  st_as_sf(velo %>% arrange(gravite), coords = c("lon", "lat"), crs = 4326, agr = "constant")
...
Cette carte montre un cercle pour chacun des `r velo %>% filter(gravite =="Mortel") %>% tally()`accidents mortels impliquant des `r type ` dans la  `r villetexte ` survenu entre 2011-2016 et géolocalisés avec succès.   Lorsque plusieurs accidents arrivent au même endroit, on n'en voit qu'un seul.
...
map_data %>% filter(gravite == "Mortel") %>% leaflet(options = leafletOptions(maxZoom = 17))%>%
  ...
  addLegend("bottomleft",
            pal = mypal,
            values = ~ gravite,
            title = paste0("Accidents mortels impliquant des vélos," ,villetexte, " 2011-2016"))
```
...
```{r carte_piste}
  ...
  localisation <- read_csv("./data/localisationcompteursvelo2015.csv" ,  locale = locale(encoding = "ISO-8859-1"))
  shp <- st_read("./data/Reseau_cyclable_2017_juin2017.shp")%>% st_transform(., "+proj=longlat +datum=WGS84") %>%  st_zm()
...
```{r}
# find nearest line from points.  pour trouver quel segment de piste cyclable correspond à chaque compteur.
...
localisation2 <-  st_as_sf(localisation  , 
                      coords = c("coord_X", "coord_Y"), 
                      crs = 4326, agr = "constant") 

pointsWgs84.sf <- st_transform(localisation2, crs = 4326)
lineWgs84.sf <- st_transform(shp, crs = 4326)

# Calculate distances -----------------------------------------------------
### mais ca fait une ligne avec toutes les pistes.. moi je veux trouver le segment de piste le plus proche..
dist <- geosphere::dist2Line(p = st_coordinates(pointsWgs84.sf), 
                             line = st_coordinates(lineWgs84.sf)[,1:2])

class(st_coordinates(lineWgs84.sf)[,1:2])
st_coordinates(pointsWgs84.sf)




#https://stackoverflow.com/questions/41234566/using-map-in-nested-data-frame
# this runs
 # lines <- st_coordinates(lineWgs84.sf %>% slice(1:10)) %>%  as.tibble() %>% group_by(L2)  %>%nest()
 #  z <- lines %>% mutate(dist = map(data, ~geosphere::dist2Line(p = st_coordinates(pointsWgs84.sf), 
 #                               line = as.matrix(.x  %>% select(X,Y))) ))

#this runs too  
#geosphere::dist2Line(p = st_coordinates(pointsWgs84.sf), 
#line = as.matrix(lines %>% slice(1:1) %>% unnest() %>% select(X,Y)))


# i wish this ran too
z <- lines %>% mutate(dummy= TRUE) %>% left_join(st_coordinates(pointsWgs84.sf) %>% as_tibble() %>% mutate(dummy=TRUE)) 
