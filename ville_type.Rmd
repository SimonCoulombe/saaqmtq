...
prep_csv_data2017 <- function(.data, .ville, .type){
  accidents <- .data
  ...
  accidents$ACCDN_PRES_DEmod <- accidents$ACCDN_PRES_DE# replace short street names by full name, fix a few typos.
...
pouet <- prepared_2011_2016 %>% left_join( velo %>% select(id, google_lat = lat , google_lon = lon)) 
...
```{r geocode_2017}
prepared_2011_2016 <- prep_csv_data2017(.data= accidents, .ville = "province", .type = "vélos")
prepared_2017 <- prep_csv_data(.data= accidents17, .ville = "province", .type = "vélos")

villes_2011_2016 <- prepared_2011_2016 %>% distinct(ville)
villes2017 <- prepared_2017 %>% distinct(ville)

get_villes_bounding_boxes <- function(.villes){
  ...
  output %>% select(ville, everything())
}

bounding_boxes2017 <- get_villes_bounding_boxes(villes2017)
write_rds(bounding_boxes2017, "bounding_boxes2017.rds")

all_binding_boxes <- bind_rows(bounding_boxesunique2011 ,bounding_boxes2017)
write_rds(all_binding_boxes, "all_binding_boxes.rds")


get_ville_google_centers <- function(.villes){
  centers <- ggmap::geocode(location = .villes %>% pull(ville), output = "latlon", source= "google")
  .villes %>% add_column(ville_lon = centers$lon) %>% add_column(ville_lat = centers$lat)
}

villes_centers_unique  <- get_ville_google_centers(villes_2011_2016 %>% anti_join(villes_centers_2017))
villes_centers_2017 <- get_ville_google_centers(villes2017)


write_rds(bind_rows(villes_centers_unique,villes_centers_2017), "all_villes_centers.rds")

prepared_2017 <- prepared_2017 %>% 
  left_join(bounding_boxes2017) %>%
  left_join(villes_centers_2017 )
...
write_rds(data_final_2017,"data_final_2017.rds")
